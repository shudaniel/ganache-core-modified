...
Provider.prototype._queueRequest = function(payload, intermediary) {
  ...
  if (this.immediateProcessing ) {
    ...
    if ( payload.method !== "eth_call" && payload.method !== "eth_sendTransaction") {
      // Process the transaction just received if it is not a call or a send
      this._processRequestIndex( this._requestQueue.length - 1 );
    }
  }
};

Provider.prototype._flipImmediateProcessing = function(bool) {
  this.immediateProcessing = bool;
}
...
Provider.prototype._getRequestQueue = function() {
  const self = this;
  if (!self._requestQueue) {
    self._requestQueue = [];
  }

  return JSON.stringify(self._requestQueue);
}

Provider.prototype._processRequestIndex = function(idx) {
  const self = this;
  ...
  self._requestInProgress = true;
  if ( self._requestQueue.length <= idx) {
    // Invalid index
    // Free lock
    self._requestInProgress = false;
    return
  ...
  // console.log(self._requestQueue);
  const args = self._requestQueue[idx];
  // console.log(args);
  self._requestQueue.splice(idx, 1);
  if (args) {
    self.engine.sendAsync(args.payload, (err, result) => {
      args.callback(err, result);
    });
