      ...
      .on("end", function() {
        ...
        switch (method) {
          ...
          case "GET":
            ...
            const request_type = query.method
            if (request_type === "eth_flipImmediateProcessing") {
              const newbool = query.flipbit === "true" ? true : false;
              provider._flipImmediateProcessing(newbool);
              sendResponse(response, 200, headers, "Success");
            }
            else if (request_type === "eth_getQueue") {
              headers["Content-Type"] = "application/json";
              sendResponse(response, 200, headers, provider._getRequestQueue());
            } else if (request_type === "eth_popQueue") {
              // Get the index
              // console.log(JSON.stringify(query, null, 4))

              const index = query.index;
              if (isNaN(index)) {
                sendResponse(response, 400, headers, "400 Bad Request: Not a number");
              }
              else {
                provider._processRequestIndex(parseInt(index));
                sendResponse(response, 200, headers, "Success");
              }
            }
            else {
